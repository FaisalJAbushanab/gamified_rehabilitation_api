...
from database import (
    ...
    add_or_update_word_history, get_user_practiced_word_ids,
    get_user_word_statistics, reset_user_word_history
)
...
@app.get("/api/users/{user_id}/practiced-words")
async def get_practiced_words(user_id: int):
    """Get list of word IDs that user has practiced"""
    try:
        practiced_word_ids = get_user_practiced_word_ids(user_id)
        return {"success": True, "practiced_word_ids": practiced_word_ids}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/users/{user_id}/unpracticed-words")
async def get_unpracticed_words(user_id: int, frequency_level: Optional[int] = None):
    """Get words that user hasn't practiced yet, optionally filtered by frequency level"""
    try:
        practiced_word_ids = get_user_practiced_word_ids(user_id)
        
        # Filter out practiced words
        unpracticed = [
            word for word in WORDS_DATABASE
            if word["id"] not in practiced_word_ids
        ]
        
        # Optionally filter by frequency level
        if frequency_level is not None:
            unpracticed = [
                word for word in unpracticed
                if word.get("frequency_level") == frequency_level
            ]
        
        return {"success": True, "words": unpracticed, "count": len(unpracticed)}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/users/{user_id}/word-statistics/{word_id}")
async def get_word_stats(user_id: int, word_id: int):
    """Get statistics for a specific word for a user"""
    try:
        stats = get_user_word_statistics(user_id, word_id)
        return {"success": True, "statistics": stats}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/api/users/{user_id}/record-word-attempt")
async def record_word_attempt(
    user_id: int,
    word_id: int = Form(...),
    result: str = Form(...),
    cue_level: int = Form(...),
    response_time_ms: int = Form(...)
):
    """Record a word attempt in user's word history"""
    try:
        add_or_update_word_history(user_id, word_id, result, cue_level, response_time_ms)
        return {"success": True, "message": "Word attempt recorded"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/api/users/{user_id}/reset-word-history")
async def reset_word_history(user_id: int):
    """Reset word history for a user"""
    try:
        reset_user_word_history(user_id)
        return {"success": True, "message": "Word history reset successfully"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
